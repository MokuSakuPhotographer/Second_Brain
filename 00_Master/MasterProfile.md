
## AI Summary

MokuSakuは、埼玉県北部在住の31歳のホテルマン。現在は、撮影技術の向上、Obsidian環境の構築、自律対話型機械ペット「シエル」の開発、偽ECサイトやWeb完結型ARGの制作、生活・資産管理の体系化に注力している。

写真分野では、人物撮影・コスプレ撮影・スナップ撮影・ブツ撮りを中心に活動している。日常の瞬間を切り取り、光と影、コントラスト、瞬間性を重視する撮影スタイルを持つ。スナップ撮影では軽量機材を、コスプレイベントやスタジオ撮影ではフル機材を使い分ける。現在の重点課題は、人物撮影におけるライティング技術、2灯ライティング、自然な表情を引き出す会話技術、後幕シンクロを活用した表現の習得である。

Obsidianは、単なるメモではなく、自分の情報・機材・目標・支出・学習内容を管理するSecond Brainとして構築している。Templater、Dataview、AI連携を活用し、サブスクリプション管理、長期目標、機材管理、日々の思考整理を体系化することを目指している。

英語学習はA2レベル。海外の人との交流を深めること、英語を知識として自分のものにすることを目的としている。Duolingo、海外YouTube、ChatGPTを使用して学習しており、関係代名詞に苦手意識がある。英語だけでなく、言語の由来、発音、自然な会話表現にも関心がある。長期的には、英語で日常会話ができるレベルを目指している。

仕事ではホテルフロント業務を担当しており、電話対応、予約受付、チェックイン・チェックアウト対応、領収書発行などの事務作業を行う。Excelを使ったリスト作成、ホテル内ポップアップ制作、写真撮影など、現場業務を便利にするための環境構築にも関心がある。

現在の主な関心は、アニメ視聴、ObsidianとAIの連携、自律対話型機械ペットの開発、新型CX-5への乗り換えである。[[2026 MAZDA CX-5 G 2WD EX Package]]を契約済みで、2026年8月お盆過ぎから下旬ごろの納車を見込んでいる。

現在抱えている課題は、人物撮影におけるライティング配置の最適解がまだ明確でないこと、友人との適切な距離感、体重が増加しないことである。2026年7月1日時点の体重は39.0kgで、短期目標として2026年9月までに体重42kg、中期目標として2027年6月までに体重45kgを目指している。シエル開発では、体験面の完成度は上がっている一方、認証情報管理、コード分割、設定管理、外部依存の安定性が課題になっている。

価値観として、実用性、コストパフォーマンス、長期使用、情報の正確性、信頼性、自分に必要なものであることを重視する。根拠のない主張、コストに見合わないもの、実用性の低い機能、ふわっとした説明を嫌う。物事を選ぶ際は、流行よりも再現性を重視し、仕様値や運用性を比較して判断する傾向がある。

AIには、結論を先に示し、手順と理由を段階的に説明することを求める。抽象論より具体例を重視し、必要に応じて表や箇条書きを使用することを好む。専門用語は解説付きで説明する必要がある。回答では、正確性、実用性、再現性、コストパフォーマンス、長期運用のしやすさを優先し、根拠のない断定、精神論、曖昧な説明、最新情報を無視した回答、メリットだけの説明、現実的でない提案は避ける。

---

# 基本情報

## 名前

- MokuSaku

## 年齢

- 31

## 誕生日

- 1995-06-02
## 職業

- ホテルマン

## 居住地

- 埼玉県北部

## 家族構成

- 母・父

---

# 現在の状況

## 現在力を入れていること

- 偽ECサイトの構築
	- index、ABOUT US、商品カード、価格帯、カート画面などを詰めている
- Web完結型ARG制作
	- 写真・Exif・レンズ情報を使ったミステリー構成を検討している
- 撮影技術の向上
	- 人物撮影におけるライティングの研究
	- 2灯ライティングの検討
	- スナップ撮影の時に光と影に注目し、写真を撮ること
- Obsidian構築
- シエル AIペット化計画
	- Tapo C200、Ollama、GPT-SoVITS、Discord連携、長期記憶、ムード管理、ダッシュボードを組み合わせている
	- 音声対話、認証情報管理、コード分割、設定管理、安定運用が今後の重点

## 最近興味があること

- アニメ視聴
- ObsidianとAIの連携
- 言語・発音の仕組み
- 新型CX-5への乗り換え
	- [[2026 MAZDA CX-5 G 2WD EX Package]]契約書類提出済み
	- 納車は2026年8月お盆過ぎ〜下旬見込み

## 今抱えている課題

- ライティングの配置は何が正解なのかが分からないこと
- 友人との距離間
	- 自分がされたら嫌なことに対して、関係を壊さずにそれをどうやって当人に伝えるか
- 体重が増加しない
	- 2026-07-01時点で39.0kg
- シエル開発の安全性・保守性
	- 認証情報の管理、巨大化した本体コード、設定の分離、外部依存の安定性が課題

---

# 目標

## 短期目標（1ヶ月～3ヶ月）

- 撮影技術向上
	- 主にライティングと人物撮影
- シエルの安全性・保守性改善
	- 認証情報を環境変数へ移す
	- 本体コードを機能別に分割する
	- 設定をJSONまたは.envに寄せる
- 偽ECサイトのカート画面追加とスマホ最適化
- 2026年9月までに体重42kgに到達
	- 毎日1個おにぎりを追加

## 中期目標（半年～1年）

- 部屋を綺麗にする
- 撮影環境の充実
- シエルを安定運用できる生活空間内AIペットとして育てる
- 2027年6月までに体重45kgに到達

## 長期目標（3年以上）

- 撮影技術を駆使した職業に就くこと
	- まだ検討段階なので目標というよりは希望的観測としている
- 英語で日常会話くらいは話せるようになること
- 2029年までに体重49kg到達

---

# 英語学習

## 現在のレベル

- A2レベル
	- 簡単な会話レベル。日常会話はまだ厳しい
	- 簡単なフレーズや文章も瞬時に出てこない時があるレベル

## 学習目的

- 海外の人との交流を深めたい
- 英語を単純な知識として自分のものにしたい

## 学習方法

- Duolingo
- Youtubeで海外の動画を見る
- わからない単語やフレーズ・文章をChatGPTに聞く

## 苦手な分野

- 関係代名詞
	- 何度も学習しているが理解できる気がしない
	- 理解できたとしても会話でスラスラ使えるビジョンが見えてこない

## よく使う教材

- Duolingo
- Youtube

---

# 写真

## 撮影ジャンル

- 人物撮影
	- ポートレート
	- コスプレ撮影
- ブツ撮り

## 撮影スタイル

- 日常を切り取る
	- 瞬間的な出来事を重視する
	- 光と影を観察し、コントラストを重要視する
- スナップ撮影では軽量機材を持っていく
- コスプレイベントやスタジオ撮影ではフル機材を持っていく
- ほぼ常にカメラを持ち歩く

## 得意分野

- スナップ撮影
- 作品に近いレタッチ
	- 独特なレタッチ

## 今後学びたいこと

- ストロボを使用したライティング
- 被写体に自然になってもらうための撮影しながらの会話技術
- 後幕シンクロを使用した撮影

---

# カメラ機材

## カメラボディ

- [[DJI Osmo Action 5 Pro]]
- [[SONY α7Ⅳ]]
- [[SONY α7Ⅴ]]
- [[SONY α7C]]

## レンズ

- [[SIGMA Art 85mm F1.4 DG DN]]
- [[SONY SEL35F18F]]
- [[SONY SEL55F18Z]]
- [[SONY SEL90M28G]]
- [[TAMRON 16-30mm F／2.8 Di III VXD G2 (A064S)]]
- [[TAMRON 28-200mm F／2.8-5.6 Di III RXD (Model A071)]]
- [[TAMRON 35-100mm F2.8 Di III VXD (Model A078S)]]
- [[TAMRON 70-180mm F／2.8 Di III VC VXD G2]]

## ストロボ

- [[GODOX AD100Pro Ⅱ]]
- [[GODOX AD100Pro Ⅱ 2台目]]
- [[GODOX TT685SⅡ-S]]

## その他機材

- [[HAKUBA E-ドライボックス KED-P20]]
- [[HAKUBA クリーニングペーパー100]]
- [[HAKUBA レンズペン3]]
- [[Kenko クリーニングキット]]
- [[Kingston SDXCカード 256GB Canvas React Plus]]
- [[Kingston SDメモリー カードリーダー]]
- [[Lexar SDXCカード Professional 1800x 256GB]]
- [[ProGrade Digital GOLD メモリーカード 64GB]]
- [[SanDisk microSD カード 256GB]]
- [[サンワダイレクト ガジェットポーチ]]
- [[Adobe After Effects]]
- [[Adobe Illustrator]]
- [[Adobe Lightroom Classic]]
- [[Adobe Photoshop]]
- [[Adobe Premiere Pro]]
- [[Apple iPad Pro 第5世代 12.9インチ Wi-Fi]]
- [[Apple iPhone 15 Pro 256GB]]
- [[Blender]]
- [[BEACO レインカバー Lサイズ]]
- [[HAKUBA プラスシェル シティロールトップバックパック26]]
- [[HAKUBA プラスシェル ショルダーバッグ M]]
- [[HAKUBA プラスシェル メッセンジャー L]]
- [[Hemmotop 三脚ケース 100cm／21cm／21cm R10021]]
- [[Hemmotop 三脚ケース 80cm ／ 18cm ／ 18cm]]
- [[Manfrotto カメラローラーバッグ Advanced III]]
- [[Panasonic エネループ単3 4本パック 大容量モデル]]
- [[SONY リチャージャブルバッテリーパック NP-FZ100]]
- [[ヘーネル ProCube2]]
- [[H&Y NDフィルター ビデオNDキット EVO マグネット式]]
- [[H&Y ステップアップリング 55-77mm マグネット式]]
- [[H&Y ステップアップリング 62-77mm マグネット式]]
- [[H&Y ステップアップリング 67-77mm マグネット式]]
- [[H&Y ナチュラルCPLフィルター EVO マグネット式]]
- [[H&Y フィルターリアキャップ 77mm EVO マグネット式]]
- [[H&Y ブラックミスト 1／4 EVO マグネット式]]
- [[H&Y レンズキャップ 77mm EVO マグネット式]]
- [[HAKUBA 55mm CPLフィルター]]
- [[JJC 折りたたみ式 フィルターケース 3枚用]]
- [[K&F Concept NDフィルター 67mm 可変式]]
- [[K&F Concept フィルターケース 10枚用]]
- [[Kenko 77mm CPLフィルター PRO1D NEO]]
- [[Kenko 可変NDフィルター 77mm ND3-400]]
- [[MARUMI 67mm CPLフィルター]]
- [[Amconsure LEDリングライト]]
- [[Godox AK-R16 磁気マウントディフューザー・カラーフィルターセット]]
- [[Godox ML-CD15]]
- [[Godox S2 ブラケット Bowensマウント]]
- [[Godox S3 ブラケット Bowensマウント]]
- [[Godox X3S]]
- [[HIPPO REILLY A3 レフ板]]
- [[NEEWER 65cm 八角形ソフトボックス オクタゴン Bowensマウント]]
- [[NEEWER 90cm八角形ソフトボックス オクタゴン Bowensマウント]]
- [[PULUZ 撮影ボックス 60x60x60cm]]
- [[Selens 撮影用 レフ板 80cm]]
- [[UNPLUGGED STUDIO 33インチ ホワイトアンブレラ]]
- [[VL-81 LEDビデオライト]]
- [[ローラーバッグのパッキング]]
- [[Carry Speed F3プレート]]
- [[Cobby カメラストラップ]]
- [[DJI RSC2]]
- [[FOSOTO ライトスタンド アルミ製]]
- [[Fotopro C-4i]]
- [[Godox FG-100]]
- [[K&F CONCEPT レフ板 クリップ]]
- [[Peak Design カフ リストストラップ]]
- [[PeakDesign アンカーリンクス]]
- [[PGYTECH クイックリリースクランプ V2]]
- [[PGYTECH ショルダーストラップ クイックリリースクランプ]]
- [[NEEWER QRP-5 38mm クイックリリースプレート 2パック]]
- [[SIRUI Traveler X Ⅲ]]
- [[SmallRig DJI RSC2用ハンドグリップ]]
- [[SPIDER CAMERA HOLSTER]]
- [[TELESIN ネックレスマウント]]
- [[ULANZI F38回転ズレ防止クイックリリースプレート]]

---

# PC環境

## [[G-Master Spear Z790 D5|メインPC]]

- [[Asetek 670LS 240mm水冷]]
- [[ASUS DRW-24D5MT]]
- [[ASUS TUF RTX 4090]]
- [[CoolerMaster CM694 TG]]
- [[Core i9 14900KF]]
- [[Crucial MX500 2TB]]
- [[Crucial T500 1TB]]
- [[FSP HYDRO GT PRO 1000W]]
- [[G-Master Spear Z790 D5]]
- [[Noctua NT-H1]]
- [[RAM 64GB]]
- [[Seagate BarraCuda 8TB]]
- [[Windows 11 Home]]
- [[Z790 Pro RS]]

## 周辺機器

- [[audio-technica AT2020]]
- [[Creative Pebble Pro]]
- [[Sennheiser HD 598]]
- [[Sony INZONE Buds]]
- [[YAMAHA AG03]]
- [[ELECOM HS-MC05BK]]
- [[Apqfw HM05-5010 PCスピーカー]]
- [[Tapo C200 製品仕様詳細|Tapo C200]]
- [[マイクアーム B07D33F6JK]]
- [[ELECOM VK720AL]]
- [[iClever IC-KP08]]
- [[Keychron K2]]
- [[Logicool MX KEYS S KX800]]
- [[BenQ EW2780U]]
- [[Green House GH-AMC03]]
- [[Pixio Prime NEO 27インチ WQHD]]
- [[UGREEN HDDケース]]
- [[Logicool G PRO X SUPERLIGHT]]
- [[Logicool MX Master 3S]]
- [[Razer DeathAdder V3 HyperSpeed]]
- [[Razer HyperPolling Wireless Dongle]]
- [[SteelSeries QcK Edge XL]]

## 使用ソフト

- [[Adobe After Effects]]
- [[Adobe Illustrator]]
- [[Adobe Lightroom Classic]]
- [[Adobe Photoshop]]
- [[Adobe Premiere Pro]]
- [[Blender]]

---

# ゲーム

## 現在プレイ中

- モバイル
	- 崩壊スターレイル
	- ツイステッドワンダーランド
	- 学園アイドルマスター
	- 鳴潮
- PC
	- なし

## 特に好きなゲーム

- VALORANT
- League of Legends
- Grand Theft Autoシリーズ
- Cyberpunk 2077
- シミュレーションゲーム全般
	- Cities Skylinesシリーズ
	- Simsシリーズ
	- Football managerシリーズ
	- Euro Truck Simulator 2
	- Stardew Valley
- ストラテジーゲーム全般
	- Victoria
	- 
- No Man's Sky
- Ready or Not
- モンスターハンターシリーズ
- 

---

# ホームシアター

## 現在の構成

- 5.1.2ch
	- [[D-109M]]
		- サラウンドスピーカー・ハイトスピーカー
	- [[D-509C]]
		- センタースピーカー
	- [[D-509E]]
		- フロントスピーカー
	- [[KJ-55X9000E]]
		- テレビ
	- [[SL-A251]]
		- サブウーファー
	- [[TX-NR646]]
		- AVアンプ
	- [[Z6 Polar Meets popIn Aladdin]]
		- プロジェクター

## 今後の構成予定

- 現在の構成のまま更新予定無し

## 課題

- 定位感や音場の詰めが甘い

---

# 仕事

## 業務内容

- フロント業務全般
	- 電話対応
	- 予約受付
	- チェックイン・アウト対応
	- 領収書などを発行する事務全般
	- 会社内でのより便利な環境構築

## よく使うスキル

- Excelを使用した簡単な氏名リスト作成
- クリエイティブを活かしたホテル内ポップアップ作成
- ホテル内のポップアップなどに使う写真の撮影


---

# 価値観

## 重視すること

- 実用性
- コストパフォーマンス
- 長期間使用できる
- 情報の正確性
- 信頼性
- 自分に必要なものであること

## 苦手なこと

- 根拠のない主張
- コストに見合わないもの
- 自分にとって実用性の低い機能
- ふわっとした説明
- 

## 物事を選ぶ基準

- 実用性を最優先する
- コストに対する効果を重視する
- 自分で管理できる仕組みを好む
- 流行より再現性を重視する
- 趣味でも学習効果があるものを好む
- 候補が複数ある場合は仕様値を比較してより良い方を選ぶ
- 

---

# AIへの指示

## 回答スタイル

- 結論を先に示す
- 手順を段階的に説明する
- 理由も併せて説明する
- 抽象論より具体例を重視する
- 必要に応じて表や箇条書きを使う
- 専門用語は解説付きで説明する

## 優先事項

- 情報の正確性
- 実用性
- 再現性
- コストパフォーマンス
- 長期運用のしやすさ

## 回避してほしいこと

- 根拠のない断定
- 精神論や気合論
- ふわっとした説明
- 最新情報を無視した回答
- メリットだけを説明すること
- 現実的ではない提案

---
# サブスクリプション

 [[サブスク集計]]を参照。
 
---

# 更新履歴


| 2026-06-06 | MasterProfile作成  |
| ---------- | ---------------- |
| 2026-06-10 | AI Summary、目標の更新 |
| 2026-07-12 | 2026-06-01〜2026-07-01更新ノートをもとに現在の状況、目標、課題、AI Summaryを更新 |
