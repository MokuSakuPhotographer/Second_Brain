---
type: pc_part
名前: GeForce RTX 4090
カテゴリ: PC環境
メーカー: NVIDIA
種類: GPU
用途: 制作作業・表示出力
所有状況: 使用中
詳細確認状況: 未確認
公式仕様確認: 済
実機ボードメーカー: 未確認
CUDAコア: 16384
アーキテクチャ: Ada Lovelace
ベースクロック: 2.23GHz
ブーストクロック: 2.52GHz
メモリ容量: 24GB
メモリ種別: GDDR6X
メモリバス幅: 384-bit
PCIe: PCI Express Gen 4
AV1エンコード: 対応
source_urls:
  - "公式仕様: https://www.nvidia.com/en-us/geforce/graphics-cards/40-series/rtx-4090/"
tags:
  - PC環境
  - GPU
aliases:
  - RTX 4090
  - NVIDIA GeForce RTX 4090
  - GeForce 4090
関連項目:
  - "[[PC環境|PC環境]]"
  - "[[PC環境 - Core i9 14900KF|Core i9 14900KF]]"
  - "[[PC環境 - Pixio Prime NEO 27インチ WQHD|Pixio Prime NEO]]"
  - "[[PC環境 - BenQ EW2780U|BenQ EW2780U]]"
  - "[[Blender|Blender]]"
  - "[[Adobe Premiere Pro|Premiere Pro]]"
  - "[[Adobe After Effects|After Effects]]"
---

# GeForce RTX 4090

#PC環境 #GPU

## 基本情報

- **カテゴリ**: PC環境
- **メーカー**: NVIDIA
- **種類**: GPU
- **用途**: 制作作業・表示出力
- **所有状況**: 使用中

## 使用詳細

- [[PC環境|PC環境]] のメインGPU。
- [[Blender|Blender]]、[[Adobe Premiere Pro|Premiere Pro]]、[[Adobe After Effects|After Effects]] などの制作作業で使用。
- 接続モニターは [[PC環境 - Pixio Prime NEO 27インチ WQHD|Pixio Prime NEO]] と [[PC環境 - BenQ EW2780U|BenQ EW2780U]]。
- ボードメーカー、接続端子、ドライバ設定、電源構成は未確認。

## 仕様

| 項目 | 内容 |
|---|---|
| アーキテクチャ | Ada Lovelace |
| CUDAコア | 16,384 |
| ベースクロック | 2.23GHz |
| ブーストクロック | 2.52GHz |
| メモリ | 24GB GDDR6X |
| メモリバス幅 | 384-bit |
| PCIe | PCI Express Gen 4 |
| AV1 | エンコード / デコード対応 |

## 調査メモ

- NVIDIAのリファレンス仕様は確認済み。
- 実際に搭載されているボードメーカー、端子数、ドライババージョン、電源接続は実機情報を取得できなかったため未確認。

## 関連項目

- [[PC環境|PC環境]]
- [[PC環境 - Core i9 14900KF|Core i9 14900KF]]
- [[PC環境 - Pixio Prime NEO 27インチ WQHD|Pixio Prime NEO]]
- [[PC環境 - BenQ EW2780U|BenQ EW2780U]]
- [[Blender|Blender]]

## メモ

- GPU支援を使う具体的なアプリ設定は未確認。
